...
# Rune line work and the scene constants the wizard stage is built from.
RUNE_SHAPES = (
...
def render(slug, title, description):
    ...
    tokens = {
        ...
        # Only the home page draws the scene, so only the home page carries
        # the sprite its rune and hex glyphs are referenced from.
        'ICON_SPRITE': icon_sprite() if slug == 'index' else '',
        'FLAME_FIELD': flame_field() if slug == 'index' else '',
        'HEX_ACCENTS': hex_accents() if slug == 'index' else '',
        'RUNE_CIRCLE': rune_circle() if slug == 'index' else '',
        'FALL_RUNES': fall_runes() if slug == 'index' else '',
        'RUNE_CLOUD': rune_cloud() if slug == 'index' else '',
        'CIRCUIT_TRACES': circuit_traces() if slug == 'index' else '',
        'FLOATING_RUNES': floating_runes() if slug == 'index' else '',
        'WIZARD_VOICE': wizard_voice() if slug == 'index' else '',
    }
